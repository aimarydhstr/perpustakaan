         ...
         <div class="relative">
            ...
            <div class="flex items-center mb-5 px-4">
               <div class="relative w-full">
                  ...
                  <input type="text" name="search" id="search" class="border bg-transparent text-sm rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 block w-full pl-10 p-2.5 dark:border-slate-500" placeholder="Cari judul buku atau nama anggota..." autofocus autocomplete="off">
               </div>
            </div>
...
$(document).ready(() => {
   ...
   function fetchData(page, search){
      $.ajax({
         url:"/laporan/peminjaman/data?page="+page+"&search="+search,
   ...
   $(document).on('change', '#kondisi', function(e){
      e.preventDefault()
      if($('#kondisi').val() == 0) $('#dendaRusak').removeClass('hidden')
      if($('#kondisi').val() == 1) $('#dendaRusak').addClass('hidden')
   })
