...
<section class="flex items-center justify-between mb-5">
   <div class="p-3 basis-1/3">
      <div class="bg-white shadow-xl flex items-center dark:bg-slate-700 p-8 rounded-xl">
         ...
         <div class="ml-5">
            <p class="font-bold text-xs uppercase mb-2 text-slate-500 dark:text-slate-200">Peminjaman Hari Ini</p>
            <p class="text-3xl font-bold text-slate-500 dark:text-slate-200">{{ $borrowDay }}</p>
         </div>
      </div>
   </div>
   <div class="p-3 basis-1/3">
      <div class="bg-white shadow-xl flex items-center dark:bg-slate-700 p-8 rounded-xl">
         ...
         <div class="ml-5">
            <p class="font-bold text-xs uppercase mb-2 text-slate-500 dark:text-slate-200">Peminjaman Bulan Ini</p>
            <p class="text-3xl font-bold text-slate-500 dark:text-slate-200">{{ $borrowMonth }}</p>
         </div>
      </div>
   </div>
   <div class="p-3 basis-1/3">
      <div class="bg-white shadow-xl flex items-center dark:bg-slate-700 p-8 rounded-xl">
         ...
         <div class="ml-5">
            <p class="font-bold text-xs uppercase mb-2 text-slate-500 dark:text-slate-200">Total Peminjaman</p>
            <p class="text-3xl font-bold text-slate-500 dark:text-slate-200">{{ $borrowTotal }}</p>
         </div>
      </div>
   </div>
</section>
...
<div class="block mb-5">
   <section class="p-3">
      <div class="bg-white dark:bg-slate-700 shadow-xl p-3 pb-7 rounded-xl relative overflow-hidden box-border">
         <div class="relative">
            <h3 class="p-4 pb-3 uppercase text-sm font-bold">Laporan Peminjaman Buku</h3>
            <p class="mb-6 text-sm px-4">Laporan data peminjaman buku pada perpustakaan</p>
         ...
         <div id="result">
            @include('laporan.peminjaman.data.index')
         </div>
...
$(document).ready(() => {
   ...
   function fetchData(page, search, firstDate, secondDate){
      $.ajax({
         url:"/laporan/peminjaman/data?page="+page+"&search="+search+"&firstDate="+firstDate+"&secondDate="+secondDate,
