   ...
   <section class="p-3">
      <div class="bg-white dark:bg-slate-700 shadow-xl p-3 pb-7 rounded-xl relative overflow-hidden box-border">
         <div class="relative">
            ...
            <p class="mb-6 text-sm px-4">Pilih anggota perpustakaan untuk meminjam buku</p>
         ...
         <div class="flex">
            <div class="basis-1/5 p-3">
               <div class="block bg-white dark:bg-slate-600 shadow-lg rounded-xl relative cursor-pointer">
                  ...
                  <div class="flex text-center justify-center items-center p-3 absolute left-0 right-0 bottom-0 bg-slate-800 bg-opacity-75 rounded-b-xl">
                     <h2 class="text-sm text-white font-bold">{{ $book->judul }}</h2>
                  </div>
            ...
            <div id="result" class="grow p-3">
               @include('peminjaman.data.member')
            </div>
         </div>
...
$(document).ready(() => {
   ...
   function fetchData(page, search){
      $.ajax({
         url:"/peminjaman/member/data?page="+page+"&search="+search,
