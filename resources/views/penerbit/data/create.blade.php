<div id="createModal" class="w-screen h-screen flex items-center justify-center bg-slate-800 bg-opacity-50 fixed top-0 left-0 right-0 bottom-0 hidden py-7">
   <div class="bg-white text-slate-700 dark:text-slate-200 dark:bg-slate-600 pb-7 rounded-xl basis-1/3">
      <p class="font-semibold py-5 px-7 text-lg border-b dark:border-slate-500 mb-3">Tambah Penerbit</p>
      
      <form method="post" action="{{ route('penerbit.store') }}">
         @csrf
         <div class="pt-3 px-7">
            <label for="nama">Nama penerbit</label>
            <input name="nama" id="nama" type="text" class="bg-transparent p-3 focus:outline-none focus:ring focus:ring-violet-500 rounded-lg w-full border dark:border-slate-500 my-3" placeholder="Ketik nama penerbit..." required autocomplete="off" autofocus>
         </div>
